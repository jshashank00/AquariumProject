...
void AquariumView::Initialize(wxFrame* parent)
{
    ...
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &AquariumView::OnAddFishBubbleFish, this, IDM_ADDBUBBLEFISH);
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &AquariumView::OnAddDory, this, IDM_ADDDORY);
 ...
 * @param event Mouse event
 */
void AquariumView::OnAddFishBubbleFish(wxCommandEvent& event)
{
    auto fish = make_shared<BubbleFish>(&mAquarium);
    mAquarium.Add(fish);
    Refresh();
}

/**
 * Menu hander for Add Fish>Dory
 * @param event Mouse event
 */
void AquariumView::OnAddDory(wxCommandEvent& event)
{
    auto fish = make_shared<Dory>(&mAquarium);
    mAquarium.Add(fish);
    Refresh();
}
...
{
    // Call Refresh() to update the frame's display
    Refresh();
}
